...
{
    /// <summary>
    /// LINE connector. See https://github.com/kenakamu/line-bot-sdk-csharp for more detail.
    /// </summary>
    public class LineMessagesController : ApiController
    {
        ...
        /// <summary>
        /// Send alert notification only
        /// </summary>
        [HttpPost]
        public async Task Notify([FromUri]string mids, [FromUri]int lcid)
        {
            Thread.CurrentThread.CurrentCulture = new CultureInfo(lcid);
            Thread.CurrentThread.CurrentUICulture = new CultureInfo(lcid);

            ButtonsTemplate buttonsTemplate = new ButtonsTemplate(text: Resource.Notification);
            buttonsTemplate.Actions.Add(new PostbackTemplateAction(Resource.Confirm, Resource.Confirm, Resource.Confirm));
            await this.Notify(new List<Message>() { new TemplateMessage(Resource.Notification, buttonsTemplate) }, mids, lcid);
        }
        ...
        /// </summary>      
        [HttpPost]
        public async Task Notify([FromUri]string message, [FromUri]string mids, [FromUri]int lcid)
        {
            await this.Notify(new List<Message>() { new TextMessage(message) }, mids, lcid);
        }
                ...
                if (activity.Attachments != null && activity.Attachments.Count != 0 && (activity.AttachmentLayout == null || activity.AttachmentLayout == "list"))
                {
                    foreach (var attachment in activity.Attachments)
                    {
                        ...
                        else if (attachment.ContentType.Contains("card.hero") || attachment.ContentType.Contains("card.thumbnail"))
                        {
                            ...
                            {
                                foreach (var button in hcard.Buttons.Take(4))
                                {
